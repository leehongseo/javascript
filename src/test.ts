...
var n:number =3;
var f:number =3.2;// 자바스크립트는 정수와 실수 따로 구분해있지 않다 
var s:string = 'abc'; // html은 "" , 자바스크립트는 ''  문법은 아니지만 규칙
var b:boolean = true; // 기본적인 데이터타입은 이 4개만 알아도 문제없다 조건문 사용할때 사용할수 있는 타입들

function test():number{
    return 1;
} // 리턴이 상당히 중요한데 그 리턴하는 값이 어떤 데이터 타입이냐가 정요하다 
function test1():void{ // void 아무것도 넘기지 않겠단느 것 
    //return 2; // 이렇게 리턴을 안해야지 에러가 나지 않는다 
}
// 리턴을 하고 안하고의 차이 : 넘버로 써있는건 대입이 되는데 void 같은 경우는 대입이 불가능하다 

var t1:number =test();
var t:any = test1(); // any는 모든것을 다 대입할수있는 타입 
